...
public class Quest7 {
    public static void main(String[] args) {
       ...
       static int maxChocoUtilRecursion(int i, int j1, int j2, int n, int m, int[][] grid) {
        ...
        for (int di = -1; di <= 1; di++) {
            for (int dj = -1; dj <= 1; dj++) {
                ...
                if (j1 == j2) {
                    ans = grid[i][j1] + maxChocoUtilRecursion(i + 1, j1 + di, j2 + dj, n, m, grid);
                }
                else {
                    // Add chocolates from both j1 and j2
                    ans = grid[i][j1] + grid[i][j2] + maxChocoUtilRecursion(i + 1, j1 + di, j2 + dj, n, m, grid);
                }
    ...
    static int maximumChocolatesRecursion(int n, int m, int[][] grid) {
        // Call the utility function to find the maximum number of chocolates
        return maxChocoUtilRecursion(0, 0, m - 1, n, m, grid);
    }
    ...
    static int maximumChocolatesTab(int n, int m, int[][] grid) {
        ...
        // Initialize the dp array with values fr om the last row of the grid
    ...
    static int maximumChocolatesTabSpace(int n, int m, int[][] grid) {
